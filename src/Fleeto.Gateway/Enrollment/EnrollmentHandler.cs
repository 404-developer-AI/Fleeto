...
/// <summary>
/// <c>POST /v1/enroll</c> (ARCHITECTURE.md §4, Enrollment): pre-checks the token, hands the request to fleeto-signer
...
public sealed class EnrollmentHandler
{
    ...
    public async Task HandleAsync(HttpContext context)
    {
        ...
        switch (outcome.State)
        {
            ...
            case SigningOutcomeState.Refused:
                _metrics.EnrollmentRefused();
                _logger.LogInformation("Enrollment from {RemoteAddress} refused by fleeto-signer", remoteAddress);
            ...
            case SigningOutcomeState.TimedOut:
                _metrics.EnrollmentFailed();
                _logger.LogWarning("Enrollment from {RemoteAddress}: fleeto-signer did not answer in time", remoteAddress);
                await ServiceUnavailableAsync(context, "Enrollment is taking longer than expected. Try again in a moment.");
                return;
            default:
                _metrics.EnrollmentFailed();
                _logger.LogWarning("Enrollment from {RemoteAddress}: fleeto-signer could not complete the request", remoteAddress);
    ...
    private void AllowNewCertificate(byte[] enrollResponse)
    {
        ...
        {
            _logger.LogWarning(ex, "The enrollment response from fleeto-signer could not be read; the certificate is allowed after the next reload");
        }
    }
