...
public sealed class ApiKeyAuthenticator
{
    ...
    public ApiKeyAuthenticator(IFleetifyDbContextFactory dbFactory, IAuditLog audit, TimeProvider time, ILogger<ApiKeyAuthenticator> logger)
    {
    ...
    public async Task<ApiAuthentication> AuthenticateAsync(string? authorization, string? ipAddress, CancellationToken cancellationToken)
    {
        ...
        return new ApiAuthentication(new Caller(key.Id, key.Name, string.Empty, [FleetifyRoles.ReadOnly], scope, ipAddress, AuditActorType.ApiKey), null);
    }
}
