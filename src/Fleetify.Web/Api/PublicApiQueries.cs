...
public sealed class PublicApiQueries
{
    ...
    public PublicApiQueries(IFleetifyDbContextFactory dbFactory, LicenseService licenses, EndpointService endpoints, EndpointCheckService checks,
    ...
    private static IQueryable<ClientRow> ProjectClients(FleetifyDbContext db, IQueryable<Client> clients) =>
    ...
    private async Task<List<ApiEndpoint>> ToApiEndpointsAsync(FleetifyDbContext db, IQueryable<Endpoint> endpoints, CancellationToken cancellationToken)
    {
        var now = _time.GetUtcNow().UtcDateTime;
