...
/// clear answer; fleetify-signer decides again before anything is signed (ARCHITECTURE.md §4, Job).
/// </summary>
public sealed class JobService
{
    ...
    public JobService(IFleetifyDbContextFactory dbFactory, LicenseService licenses, INotificationBus bus, TimeProvider time)
    {
    ...
    /// <summary>
    /// Creates one job per endpoint for the script's current version and asks fleetify-signer to sign them. Endpoints that cannot
    ...
    private static IQueryable<JobListItem> Project(FleetifyDbContext db, IQueryable<Job> jobs) =>
    ...
    private static async Task<(string Text, long Bytes, bool Shortened)> ReadStreamAsync(FleetifyDbContext db, Guid jobId, JobStream stream,
