...
public static class AccountService
{
    ...
    public const string MarkerProvider = "[Fleetify]";
...
public sealed class SetupService
{
    ...
    private readonly FleetifyDbContext _db;
    private readonly TimeProvider _time;

    public SetupService(UserManager<ApplicationUser> users, FleetifyDbContext db, TimeProvider time)
    {
        _users = users;
        _db = db;
        _time = time;
        ...
        string? ipAddress, CancellationToken cancellationToken = default)
    {
        ...
        var role = await _users.AddToRoleAsync(user, FleetifyRoles.Admin);
        if (!role.Succeeded)
        {
            return new SetupResult(state, SetupProblem.AccountRefused, null);
        }
