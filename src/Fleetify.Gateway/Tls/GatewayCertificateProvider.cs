...
/// <summary>
/// Obtains the gateway server certificate from fleetify-signer. The private key is generated in memory at start and
...
public sealed class GatewayCertificateProvider : BackgroundService
{
    ...
    private volatile string _status = "Requesting a gateway certificate from fleetify-signer.";
    ...
    internal async Task<bool> ObtainAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            var request = new CertificateRequest("CN=fleetify-gateway, O=Fleetify", key, HashAlgorithmName.SHA256);
            ...
            switch (outcome.State)
            {
                case SigningOutcomeState.TimedOut:
                    return Fail("fleetify-signer did not answer the gateway certificate request in time. Check that fleetify-signer is running.");
                case SigningOutcomeState.Refused:
                    return Fail($"fleetify-signer refused the gateway certificate request: {outcome.RefusalReason}");
                case SigningOutcomeState.Failed or SigningOutcomeState.Completed when outcome.Result is null:
                    return Fail("fleetify-signer could not issue a gateway certificate. Check the fleetify-signer log.");
            }
    ...
    private async Task<bool> InstallAsync(byte[] certificateDer, ECDsa key, CancellationToken cancellationToken)
    {
        ...
        {
            return Fail("The certificate from fleetify-signer does not match the gateway key. It was not installed.");
        }
        ...
        if (chain is null)
        {
            return Fail("The certificate from fleetify-signer does not chain to an instance CA. It was not installed.");
        }
