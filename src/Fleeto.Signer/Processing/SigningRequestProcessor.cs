...
public sealed class SigningRequestProcessor
{
    ...
    private const string HandlerSavepoint = "fleeto_signer_handler";
    private const string SignerActor = "fleeto-signer";
    ...
    public SigningRequestProcessor(IFleetoDbContextFactory dbFactory, SignerKeyRing keyRing, IEnumerable<ISigningRequestHandler> handlers,
    ...
    private async Task<(SigningRequest Request, SigningOutcome Outcome)> RunHandlerAsync(FleetoDbContext db, IDbContextTransaction transaction,
    ...
    private static async Task<SigningRequest?> ClaimAsync(FleetoDbContext db, Guid? requestId, IReadOnlySet<SigningRequestKind> waitingKinds,
