...
namespace Fleeto.Workers.Configuration;
...
/// Woken by <c>fleeto_config_changes</c>, and polls every 30 seconds so a lost notification only delays work.
/// </summary>
public sealed class ConfigChangeFanoutService : WorkerLoop
{
    ...
    public ConfigChangeFanoutService(IFleetoDbContextFactory dbFactory, INotificationBus bus, WorkerHeartbeat heartbeat,
    ...
    private async Task<string> SelectStatementAsync(FleetoDbContext db, ConfigChangeEvent change, CancellationToken cancellationToken)
    {
