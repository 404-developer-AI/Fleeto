...
/// </para>
/// <para>
/// Work arrives through <c>fleeto_check_results</c> (payload endpoint id) and a sweep every 30 seconds over recent
...
public sealed class CheckEvaluationService : WorkerLoop
{
    ...
    public CheckEvaluationService(IFleetoDbContextFactory dbFactory, INotificationBus bus, LicenseService licenses,
    ...
    /// </summary>
    private static async Task<bool> ApplyResultsAsync(FleetoDbContext db, EndpointFacts endpoint, List<CheckResult> results,
    ...
    private static async Task UpsertRollupsAsync(FleetoDbContext db, string table, EndpointFacts endpoint,
