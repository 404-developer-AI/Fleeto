...
namespace Fleeto.Workers.Jobs;

/// <summary>
/// Moves jobs that nobody will finish to their final state (0.2.0, ARCHITECTURE.md §4, Job and Job output), every minute:
/// <list type="bullet">
/// <item>a job fleeto-signer did not sign within 15 minutes is refused (the signer was down; the technician starts it again);</item>
...
{
    public const string SignerTimeoutReason = "fleeto-signer did not sign the job in time. Start the job again; check that fleeto-signer is running.";
    ...
    public JobMaintenanceService(IFleetoDbContextFactory dbFactory, INotificationBus bus, WorkerHeartbeat heartbeat, TimeProvider time,
