...
public sealed class SigningKeyBootstrapper
{
    ...
    public SigningKeyBootstrapper(IFleetifyDbContextFactory dbFactory, SignerKey signerKey, SignerKeyRing keyRing, TimeProvider time,
    ...
    public async Task RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        var instance = await db.InstanceSettings.AsNoTracking().SingleOrDefaultAsync(cancellationToken)
            ?? throw new InvalidOperationException("The instance is not initialised. Run fleetify-tool migrate first.");
        ...
        if (signing is null)
        {
            ...
            await SignerAudit.WriteAsync(db, new AuditRecord(SigningKeyCreatedAction, "InstanceSigningKey", id, null,
                AuditActorType.System, "fleetify-signer", "fleetify-signer", new { KeyId = id }), now, cancellationToken);
        ...
        if (ca is null)
        {
            ...
            await SignerAudit.WriteAsync(db, new AuditRecord(CertificateAuthorityCreatedAction, "CertificateAuthority", id.ToString(), null,
                AuditActorType.System, "fleetify-signer", "fleetify-signer", new { material.Fingerprint, material.ExpiresAt }), now, cancellationToken);
