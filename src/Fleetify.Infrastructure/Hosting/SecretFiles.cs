...
{
    public const string SectionName = "Fleetify:Secrets";

    /// <summary>
    /// Directory with the secret files. Empty means: the <c>FLEETIFY_SECRETS_DIR</c> environment variable, else
    /// <c>%LOCALAPPDATA%\Fleetify\dev\secrets</c> on Windows (local development), else <c>/run/secrets</c>
...
public sealed class SecretFiles
{
    ...
    private static string ResolveDirectory(string configured)
    {
        ...
        var fromEnvironment = Environment.GetEnvironmentVariable("FLEETIFY_SECRETS_DIR");
        ...
        return OperatingSystem.IsWindows()
            ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Fleetify", "dev", "secrets")
            : "/run/secrets";
    }
}
