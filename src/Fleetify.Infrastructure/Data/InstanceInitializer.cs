...
public sealed class InstanceInitializer
{
    ...
    public InstanceInitializer(IFleetifyDbContextFactory dbFactory, RootKey rootKey, TimeProvider time, ILogger<InstanceInitializer> logger)
    {
    ...
    public async Task<InstanceInitResult> RunAsync(InstanceInitOptions options, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var roleName in FleetifyRoles.All)
        {
        ...
        var adminRoleId = await db.Roles.Where(r => r.NormalizedName == FleetifyRoles.Admin.ToUpperInvariant()).Select(r => r.Id).SingleAsync(cancellationToken);
