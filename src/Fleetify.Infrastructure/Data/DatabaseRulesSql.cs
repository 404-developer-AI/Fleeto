...
public static class DatabaseRulesSql
{
    public const string Up = """
        ...
        CREATE OR REPLACE FUNCTION fleetify_audit_append_only() RETURNS trigger LANGUAGE plpgsql AS $$
        BEGIN
        ...
        CREATE TRIGGER "TR_AuditEntries_AppendOnly" BEFORE UPDATE OR DELETE ON "AuditEntries"
          FOR EACH ROW EXECUTE FUNCTION fleetify_audit_append_only();
        CREATE TRIGGER "TR_AuditEntries_NoTruncate" BEFORE TRUNCATE ON "AuditEntries"
          FOR EACH STATEMENT EXECUTE FUNCTION fleetify_audit_append_only();

        -- ClientId is immutable on every table that carries it as the tenant boundary.
        CREATE OR REPLACE FUNCTION fleetify_client_id_immutable() RETURNS trigger LANGUAGE plpgsql AS $$
        BEGIN
          ...
          RETURN NEW;
        END $$;
        CREATE TRIGGER "TR_Sites_ClientIdImmutable" BEFORE UPDATE OF "ClientId" ON "Sites" FOR EACH ROW EXECUTE FUNCTION fleetify_client_id_immutable();
        CREATE TRIGGER "TR_Endpoints_ClientIdImmutable" BEFORE UPDATE OF "ClientId" ON "Endpoints" FOR EACH ROW EXECUTE FUNCTION fleetify_client_id_immutable();
        CREATE TRIGGER "TR_Policies_ClientIdImmutable" BEFORE UPDATE OF "ClientId" ON "Policies" FOR EACH ROW EXECUTE FUNCTION fleetify_client_id_immutable();
        CREATE TRIGGER "TR_MonitoringTemplates_ClientIdImmutable" BEFORE UPDATE OF "ClientId" ON "MonitoringTemplates" FOR EACH ROW EXECUTE FUNCTION fleetify_client_id_immutable();
        ...
        CREATE CONSTRAINT TRIGGER "TR_CheckDefinitions_Client" AFTER INSERT OR UPDATE ON "CheckDefinitions"
          FOR EACH ROW EXECUTE FUNCTION fleetify_check_definition_client();

        -- A site can link a global template or policy, or one of its own client; never another client's.
        CREATE OR REPLACE FUNCTION fleetify_site_template_client() RETURNS trigger LANGUAGE plpgsql AS $$
        ...
        CREATE CONSTRAINT TRIGGER "TR_SiteMonitoringTemplates_Client" AFTER INSERT OR UPDATE ON "SiteMonitoringTemplates"
          FOR EACH ROW EXECUTE FUNCTION fleetify_site_template_client();

        CREATE OR REPLACE FUNCTION fleetify_site_policy_client() RETURNS trigger LANGUAGE plpgsql AS $$
        ...
        CREATE CONSTRAINT TRIGGER "TR_SitePolicies_Client" AFTER INSERT OR UPDATE ON "SitePolicies"
          FOR EACH ROW EXECUTE FUNCTION fleetify_site_policy_client();

        -- Client templates apply to any client, so they may only reference global templates and policies.
        CREATE OR REPLACE FUNCTION fleetify_client_template_global() RETURNS trigger LANGUAGE plpgsql AS $$
        BEGIN
        ...
        CREATE CONSTRAINT TRIGGER "TR_ClientTemplateSites_Global" AFTER INSERT OR UPDATE ON "ClientTemplateSites"
          FOR EACH ROW EXECUTE FUNCTION fleetify_client_template_global();
        ...
        CREATE OR REPLACE FUNCTION fleetify_notify_id() RETURNS trigger LANGUAGE plpgsql AS $$
        BEGIN
          PERFORM pg_notify(TG_ARGV[0], NEW."Id"::text);
          RETURN NULL;
        END $$;
        CREATE TRIGGER "TR_SigningRequests_Notify" AFTER INSERT ON "SigningRequests"
          FOR EACH ROW EXECUTE FUNCTION fleetify_notify_id('fleetify_signing_requests');
        CREATE TRIGGER "TR_SigningRequests_NotifyResult" AFTER UPDATE OF "State" ON "SigningRequests"
          FOR EACH ROW WHEN (NEW."State" <> 'Pending') EXECUTE FUNCTION fleetify_notify_id('fleetify_signing_results');
        CREATE TRIGGER "TR_ConfigChangeEvents_Notify" AFTER INSERT ON "ConfigChangeEvents"
          FOR EACH ROW EXECUTE FUNCTION fleetify_notify_id('fleetify_config_changes');
        CREATE TRIGGER "TR_EndpointEvents_Notify" AFTER INSERT ON "EndpointEvents"
          FOR EACH ROW EXECUTE FUNCTION fleetify_notify_id('fleetify_endpoint_events');
        CREATE TRIGGER "TR_OutboxEmails_Notify" AFTER INSERT ON "OutboxEmails"
          FOR EACH ROW EXECUTE FUNCTION fleetify_notify_id('fleetify_outbox_emails');
    ...
    public const string Down = """
        ...
        DROP FUNCTION IF EXISTS fleetify_notify_id();
        ...
        DROP FUNCTION IF EXISTS fleetify_client_template_global();
        DROP TRIGGER IF EXISTS "TR_SitePolicies_Client" ON "SitePolicies";
        DROP FUNCTION IF EXISTS fleetify_site_policy_client();
        DROP TRIGGER IF EXISTS "TR_SiteMonitoringTemplates_Client" ON "SiteMonitoringTemplates";
        DROP FUNCTION IF EXISTS fleetify_site_template_client();
        DROP TRIGGER IF EXISTS "TR_CheckDefinitions_Client" ON "CheckDefinitions";
        DROP FUNCTION IF EXISTS fleetify_check_definition_client();
        ...
        DROP FUNCTION IF EXISTS fleetify_client_id_immutable();
        ...
        DROP FUNCTION IF EXISTS fleetify_audit_append_only();
        """;
}
