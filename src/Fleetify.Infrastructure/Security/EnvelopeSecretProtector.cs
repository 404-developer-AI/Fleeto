...
public sealed class EnvelopeSecretProtector : ISecretProtector
{
    ...
    public EnvelopeSecretProtector(RootKey rootKey, IFleetifyDbContextFactory dbFactory)
    {
        _rootKey = rootKey;
    ...
    /// <summary>Creates a wrapped data key for a purpose. Used by <c>fleetify-tool migrate</c> only.</summary>
    ...
    private (Guid Id, byte[] Key) GetActiveKey(string purpose)
    {
        ...
        var dataKey = db.DataKeys.AsNoTracking().SingleOrDefault(k => k.Purpose == purpose && k.RetiredAt == null)
            ?? throw new InvalidOperationException(
                $"No data key exists for '{purpose}'. Run 'fleetify-tool migrate' for this instance.");
    ...
    private static byte[] WrapAssociatedData(Guid id, string purpose) => Encoding.UTF8.GetBytes($"fleetify-dek|{id:N}|{purpose}");
