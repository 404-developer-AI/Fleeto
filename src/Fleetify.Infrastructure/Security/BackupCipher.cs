...
/// <item>HKDF-SHA256 (salt "fleetify-backup-v1", info = ephemeral public || backup public) derives a 32-byte file key.</item>
...
public static class BackupCipher
{
    ...
    private static readonly byte[] Salt = "fleetify-backup-v1"u8.ToArray();
    ...
    public static async Task EncryptAsync(Stream input, Stream output, byte[] recipientPublicKey, CancellationToken cancellationToken = default)
    {
        ...
        var fileKey = DeriveFileKey(ephemeral, new X25519PublicKeyParameters(recipientPublicKey), ephemeralPublic, recipientPublicKey);
    ...
    public static async Task DecryptAsync(Stream input, Stream output, byte[] recipientPrivateKey, CancellationToken cancellationToken = default)
    {
        ...
        var fileKey = DeriveFileKey(recipient, new X25519PublicKeyParameters(ephemeralPublic), ephemeralPublic, recipientPublic);
    ...
    private static byte[] DeriveFileKey(X25519PrivateKeyParameters privateKey, X25519PublicKeyParameters peer,
        byte[] ephemeralPublic, byte[] recipientPublic)
    {
    ...
    public static string EncodePublicKey(byte[] publicKey) => "fleetify-backup-pub:" + Convert.ToBase64String(publicKey);

    public static string EncodePrivateKey(byte[] privateKey) => "fleetify-backup-key:" + Convert.ToBase64String(privateKey);

    /// <summary>Parses a backup public key as shown in Settings or produced by <c>fleetify-tool backup keygen</c>.</summary>
    public static bool TryDecodePublicKey(string? text, out byte[] publicKey)
    {
        publicKey = [];
        const string prefix = "fleetify-backup-pub:";
        if (string.IsNullOrWhiteSpace(text) || !text.Trim().StartsWith(prefix, StringComparison.Ordinal))
        {
            return false;
        }
    ...
    {
        const string prefix = "fleetify-backup-key:";
