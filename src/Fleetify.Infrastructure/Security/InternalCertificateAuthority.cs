...
public static class InternalCertificateAuthority
{
    ...
    public static CaMaterial CreateCa(string instanceFqdn, DateTime now)
    {
        ...
        var request = new CertificateRequest($"CN=Fleeto Instance CA {Sanitize(instanceFqdn)}, O=Fleetify", key, HashAlgorithmName.SHA256);
        ...
        Guid endpointId, Guid instanceId, DateTime now)
    {
        ...
        var request = new CertificateRequest(new X500DistinguishedName($"CN={endpointId:D}, O=Fleetify"), csr.PublicKey, HashAlgorithmName.SHA256);
        ...
        san.AddUri(new Uri($"urn:fleetify:endpoint:{endpointId:D}"));
        san.AddUri(new Uri($"urn:fleetify:instance:{instanceId:D}"));
        ...
        IEnumerable<string> dnsNames, DateTime now)
    {
        ...
        var request = new CertificateRequest(new X500DistinguishedName($"CN={Sanitize(names[0])}, O=Fleetify"), csr.PublicKey, HashAlgorithmName.SHA256);
        request.CertificateExtensions.Add(new X509BasicConstraintsExtension(false, false, 0, true));
        request.CertificateExtensions.Add(new X509KeyUsageExtension(X509KeyUsageFlags.DigitalSignature, true));
        ...
        foreach (var extension in certificate.Extensions)
        {
            ...
            while (sequence.HasData)
            {
                ...
                if (tag.TagClass == TagClass.ContextSpecific && tag.TagValue == 6)
                {
                    ...
                    const string prefix = "urn:fleetify:endpoint:";
