    ...
    public partial class ScriptsAndJobs : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql("""
                CREATE TRIGGER "TR_Scripts_ClientIdImmutable" BEFORE UPDATE OF "ClientId" ON "Scripts" FOR EACH ROW EXECUTE FUNCTION fleetify_client_id_immutable();

                -- A script version carries exactly the ClientId of its script, null included.
                CREATE OR REPLACE FUNCTION fleetify_script_version_client() RETURNS trigger LANGUAGE plpgsql AS $$
                ...
                CREATE CONSTRAINT TRIGGER "TR_ScriptVersions_Client" AFTER INSERT OR UPDATE ON "ScriptVersions"
                  FOR EACH ROW EXECUTE FUNCTION fleetify_script_version_client();

                -- A job runs a global script or one of its own client; never another client's.
                CREATE OR REPLACE FUNCTION fleetify_job_script_client() RETURNS trigger LANGUAGE plpgsql AS $$
                ...
                CREATE CONSTRAINT TRIGGER "TR_Jobs_ScriptClient" AFTER INSERT OR UPDATE OF "ScriptVersionId" ON "Jobs"
                  FOR EACH ROW EXECUTE FUNCTION fleetify_job_script_client();
                ...
                BEGIN
                  ...
                  ELSIF NEW."Kind" = 'Job' THEN
                    IF current_user IN ('fleetify_gateway', 'fleetify_workers', 'fleetify_signer') THEN
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                DROP FUNCTION IF EXISTS fleetify_job_script_client();
                DROP TRIGGER IF EXISTS "TR_ScriptVersions_Client" ON "ScriptVersions";
                DROP FUNCTION IF EXISTS fleetify_script_version_client();
