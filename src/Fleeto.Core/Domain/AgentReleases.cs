...
/// <summary>
/// The part of the signed release manifest (<c>manifest.json</c>, written by <c>fleeto-tool release manifest</c>) that concerns the agent
...
public sealed partial record ReleaseManifest(string Version, IReadOnlyList<AgentBinary> AgentBinaries)
{
    ...
    [GeneratedRegex("^[a-z0-9]{1,16}-[a-z0-9]{1,16}/fleeto-(agent|watchdog)(\\.exe)?$")]
    ...
    /// <summary>The only file name a binary may have: <c>&lt;platform&gt;-&lt;architecture&gt;/fleeto-&lt;component&gt;[.exe]</c>.</summary>
    public static string ExpectedFile(AgentComponent component, string platform, string architecture) =>
        $"{platform}-{architecture}/fleeto-{(component == AgentComponent.Agent ? "agent" : "watchdog")}{(platform == "windows" ? ".exe" : string.Empty)}";
}
