...
/// pauses delivery, so a dead mail server is not hammered. Woken by <c>fleetify_outbox_emails</c>, polls every 30 seconds.
/// <para>
...
public sealed class OutboxEmailService : WorkerLoop
{
    ...
    public OutboxEmailService(IFleetifyDbContextFactory dbFactory, INotificationBus bus, IEmailTransportFactory transports,
    ...
    private async Task RecordFailureAsync(FleetifyDbContext db, Guid emailId, int previousAttempts, EmailDeliveryException failure,
