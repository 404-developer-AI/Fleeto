...
{
    public static async Task<InstanceInfo> GetInstanceAsync(FleetifyDbContext db, CancellationToken cancellationToken)
    {
        var instance = await db.InstanceSettings.AsNoTracking()
            .Select(i => new { i.InstanceId, i.Fqdn, i.WebBaseUrl })
            .SingleOrDefaultAsync(cancellationToken)
            ?? throw new InvalidOperationException("The instance is not initialised. Run 'fleetify-tool migrate' for this instance.");
    ...
    public static async Task<IReadOnlyList<string>> GetAdminEmailsAsync(FleetifyDbContext db, CancellationToken cancellationToken)
    {
        var adminRole = FleetifyRoles.Admin.ToUpperInvariant();
        var emails = await (
