...
public sealed class MaintenanceExpiryService : WorkerLoop
{
    ...
    public MaintenanceExpiryService(IFleetifyDbContextFactory dbFactory, INotificationBus bus, WorkerHeartbeat heartbeat, TimeProvider time,
        ...
        await using (var db = _dbFactory.CreateSystem())
        {
            ...
            foreach (var item in expired)
            {
                db.AuditEntries.Add(AuditLog.ToEntry(new AuditRecord(AuditActions.MaintenanceExpired, item.TargetType, item.Id.ToString(),
                    item.ClientId, AuditActorType.System, "fleetify-workers", "fleetify-workers", new { item.Name, EndedAt = item.EndsAt }), now));
