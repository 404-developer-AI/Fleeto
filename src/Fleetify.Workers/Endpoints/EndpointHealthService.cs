...
public sealed class EndpointHealthService : WorkerLoop
{
    ...
    public static string AgentStoppedTitle(string hostname, ComponentServiceState state, DateTime since) => state switch
    {
        ComponentServiceState.Stopped or ComponentServiceState.Stopping =>
            $"The Fleeto agent service on {hostname} is stopped and the watchdog cannot start it. Check the fleetify-agent service on the endpoint.",
    ...
    {
        ComponentServiceState.Stopped or ComponentServiceState.Stopping =>
            $"The Fleeto watchdog service on {hostname} is stopped and the agent cannot start it. Monitoring works, but agent updates and restarts do not; check the fleetify-watchdog service.",
        ComponentServiceState.Disabled =>
