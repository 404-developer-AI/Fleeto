...
public sealed class WalShipperService : WorkerLoop
{
    ...
    public WalShipperService(IFleetifyDbContextFactory dbFactory, SettingsStore settings, BackupDestinations destinations,
    ...
    public async Task<int> ShipPendingAsync(CancellationToken cancellationToken)
    {
        ...
        var tempDirectory = Path.Combine(string.IsNullOrWhiteSpace(_options.TempDirectory) ? Path.GetTempPath() : _options.TempDirectory, "fleetify-wal");
