...
public sealed class BackupService : WorkerLoop
{
    ...
    public BackupService(IFleetifyDbContextFactory dbFactory, SettingsStore settings, PgDumpRunner pgDump, BackupDestinations destinations,
    ...
    public async Task<BackupRun> RunBackupAsync(BackupKind kind, BackupSettings settings, CancellationToken cancellationToken)
    {
        ...
        var tempDirectory = Path.Combine(string.IsNullOrWhiteSpace(_options.TempDirectory) ? Path.GetTempPath() : _options.TempDirectory, "fleetify-backup");
