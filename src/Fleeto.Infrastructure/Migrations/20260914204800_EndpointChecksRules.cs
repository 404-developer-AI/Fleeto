    ...
    public partial class EndpointChecksRules : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                CREATE OR REPLACE FUNCTION fleeto_endpoint_template_client() RETURNS trigger LANGUAGE plpgsql AS $$
                ...
                CREATE CONSTRAINT TRIGGER "TR_EndpointMonitoringTemplates_Client" AFTER INSERT OR UPDATE OF "ClientId", "MonitoringTemplateId"
                  ON "EndpointMonitoringTemplates" FOR EACH ROW EXECUTE FUNCTION fleeto_endpoint_template_client();
                ...
                CREATE OR REPLACE FUNCTION fleeto_endpoint_check_client() RETURNS trigger LANGUAGE plpgsql AS $$
                ...
                CREATE CONSTRAINT TRIGGER "TR_EndpointCheckOverrides_Client" AFTER INSERT OR UPDATE OF "ClientId", "EndpointId", "CheckDefinitionId"
                  ON "EndpointCheckOverrides" FOR EACH ROW EXECUTE FUNCTION fleeto_endpoint_check_client();
                CREATE CONSTRAINT TRIGGER "TR_CheckRunRequests_Client" AFTER INSERT OR UPDATE OF "ClientId", "EndpointId", "CheckDefinitionId"
                  ON "CheckRunRequests" FOR EACH ROW EXECUTE FUNCTION fleeto_endpoint_check_client();
                ...
                CREATE TRIGGER "TR_CheckRunRequests_Notify" AFTER INSERT ON "CheckRunRequests"
                  FOR EACH ROW EXECUTE FUNCTION fleeto_notify_id('fleeto_check_run_requests');
                CREATE TRIGGER "TR_CheckRunRequests_NotifyReset" AFTER UPDATE OF "ResetAppliedAt" ON "CheckRunRequests"
                  ...
                  EXECUTE FUNCTION fleeto_notify_id('fleeto_check_run_requests');

                -- Checks can now also be deleted from an endpoint, so the reason no longer names the template.
                CREATE OR REPLACE FUNCTION fleeto_resolve_orphaned_check_alert() RETURNS trigger LANGUAGE plpgsql AS $$
                BEGIN
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                DROP FUNCTION IF EXISTS fleeto_endpoint_check_client();
                DROP TRIGGER IF EXISTS "TR_EndpointMonitoringTemplates_Client" ON "EndpointMonitoringTemplates";
                DROP FUNCTION IF EXISTS fleeto_endpoint_template_client();
                """);
        }
    }
}
