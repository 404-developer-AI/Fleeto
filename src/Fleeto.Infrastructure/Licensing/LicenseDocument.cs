...
/// &lt;base64 ed25519 signature over "fleeto-license-v1" || 0x00 || payload&gt;
/// -----END FLEETO LICENSE-----
/// </code>
...
public static class LicenseCodec
{
    ...
    public static LicenseVerification Verify(string? text, IReadOnlyList<byte[]> trustedPublicKeys, string expectedFqdn)
    {
        ...
        // Licenses signed before the rename to Fleeto (0.2.1) use the legacy context; both belong to the license key only.
        var key = trustedPublicKeys.FirstOrDefault(k => Ed25519.Verify(k, SignatureContexts.License, payload, signature) ||
                                                        Ed25519.Verify(k, LegacyNames.LicenseSignatureContext, payload, signature));
        if (key is null)
        {
...
{
    public static IReadOnlyList<byte[]> LicenseKeys { get; } = Read("Fleeto.LicensePublicKeys");
    public static IReadOnlyList<byte[]> ReleaseKeys { get; } = Read("Fleeto.ReleasePublicKeys");
