...
/// <summary>A certificate for an app registration: the private key stays in the instance, encrypted.</summary>
public sealed record AppCertificate(string PfxBase64, string Thumbprint, DateTime ExpiresAt);

/// <summary>
/// What every Microsoft Entra ID app registration in Fleeto shares: the endpoints of a tenant, what a tenant id and a
/// client id may look like, the certificate Fleeto creates for an app registration and the signed client assertion that
/// proves possession of its key (RFC 7523). Used by the Graph email credential (0.2.0) and by the sign-in (0.5.0).
/// </summary>
public static partial class MicrosoftIdentity
{
    ...
    public static Uri AuthorizeEndpoint(string tenantId) =>
        new($"https://login.microsoftonline.com/{Uri.EscapeDataString(tenantId)}/oauth2/v2.0/authorize");

    /// <summary>The OpenID Connect metadata of one tenant, from which the signing keys are read.</summary>
    public static Uri MetadataAddress(string tenantId) =>
        new($"https://login.microsoftonline.com/{Uri.EscapeDataString(tenantId)}/v2.0/.well-known/openid-configuration");
    ...
    public static AppCertificate CreateCertificate(string subjectName, DateTime now)
    {
        using var rsa = RSA.Create(3072);
        var request = new CertificateRequest(new X500DistinguishedName($"CN={subjectName}"), rsa, HashAlgorithmName.SHA256,
        ...
        return new AppCertificate(Convert.ToBase64String(certificate.Export(X509ContentType.Pkcs12)), certificate.Thumbprint,
