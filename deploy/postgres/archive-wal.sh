#!/bin/sh
# Fleeto: PostgreSQL archive_command. Usage: fleetify-archive-wal %p %f
#
# Copies one completed WAL segment into the spool directory shared with fleetify-workers, which encrypts it with the
...
#
# The spool is /opt/fleetify/<instance>/wal-spool on the host: owner 70 (postgres), group 10001 (workers), mode 2770.
set -eu

if [ "$#" -ne 2 ]; then
    echo "fleetify-archive-wal: expected 2 arguments (%p %f), got $#" >&2
    exit 2
fi

source_path="$1"
name="$2"
spool="/var/lib/fleetify/wal-spool"

case "$name" in
    */* | .* | "")
        echo "fleetify-archive-wal: refusing unexpected file name '$name'" >&2
        exit 2
        ;;
esac
...
if [ -f "$target" ]; then
    ...
    echo "fleetify-archive-wal: $target exists with different content; not overwriting" >&2
    exit 1
fi
