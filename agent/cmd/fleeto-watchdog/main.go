// Command fleeto-watchdog is the Fleeto watchdog (0.2.1): the second service on an endpoint that keeps the agent running and installs
...
Usage:
  fleeto-watchdog version [--short]
  fleeto-watchdog status
  fleeto-watchdog run                   (started by the service manager)

The Fleeto Agent installs, updates and removes the watchdog. Uninstall both with 'fleeto-agent uninstall'.
`
...
func run(args []string, stdout, stderr io.Writer) int {
	...
	case "version", "--version", "-v":
		...
		fmt.Fprintf(stdout, "fleeto-watchdog %s (%s/%s)\n", version.Version, runtime.GOOS, runtime.GOARCH)
		return 0
	case "status":
