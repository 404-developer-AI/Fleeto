...
| Remote session history of an endpoint: kind (remote control, remote background), started by, reason, created, started and ended with the reason, and per participant the technician, state, connected and ended times | 0.3.0 | `GET /api/v1/endpoints/{endpointId}/remote-sessions` (paged, newest first) returning `{id, kind, startedBy, reason, createdAt, startedAt, endedAt, endReason, participants: [{userName, state, connectedAt, endedAt, endReason}]}` | Managed and agent-only (history stays readable). Never tokens, keys or IP addresses. Reasons are free text. |
| Actions inside a remote background session: file, service and process actions with their target | 0.3.0 | `actions: [{time, action, target, detail}]` on a single session `GET /api/v1/remote-sessions/{sessionId}` | Written from 0.3.0 step 2. Terminal content is never stored. |
