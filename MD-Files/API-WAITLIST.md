...
| Agent and watchdog update status per endpoint: version being installed, downloading, installing, installed, failed or rolled back, with the cause | 0.2.1 | `updates: [{component, version, state, detail, at}]` on Endpoint | Additive. |
...
| Feature | Since | API shape | Notes |
|---|---|---|---|
| Policies: settings, script approval required, maintenance windows, update ring (0.2.1), which sites use them, the default policy | 0.2.1 | `GET /api/v1/policies`, `GET /api/v1/policies/{policyId}`; `policy: {id, name}` on Site | Global policies are instance-wide: a client-limited key sees only global and own-client policies. |
...
| Run a library script on endpoints (with validity), cancel a job | 0.2.0 | `POST /api/v1/jobs` `{scriptId, endpointIds, validity}`, `POST /api/v1/jobs/{jobId}/cancel` | Highest risk in the API: runs as SYSTEM or root. The signer must re-check everything as for the UI; consider a separate key permission and approval. |
