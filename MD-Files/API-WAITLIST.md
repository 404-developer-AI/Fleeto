...
| Sign-in configuration (0.5.0): saving the Entra ID tenant, client id and client secret, switching the sign-in on or off, and linking or unlinking a user | Holds a secret and decides who can sign in at all; account security belongs in the UI with two-factor authentication. Reading the configuration is on the waiting list above. |
