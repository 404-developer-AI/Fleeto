...
> Status: 0.0.x to 0.5.0 released (agent enrollment, gateway, signer, workers, web UI, licensing, backups, maintenance,
...
> installing the Action1 agent; and sign-in with Microsoft Entra ID with users from the owner's tenant). The other integrations (Sophos, Veeam, Proxmox, vCenter) are design.
...
| **ClientPolicy**, **SitePolicy**, **EndpointPolicy** | `ClientId` / `SiteId` (key with `AppliesTo`: `All`, `Server` or `Workstation`) / `EndpointId` (key), `ClientId`, `PolicyId`, `Source` (`Manual`/`ClientTemplate`, not on the endpoint), `CreatedAt` | The policy of a client, site or endpoint (client and endpoint level from 0.6.0). A client or site holds one for every endpoint (`All`) or one per class; an endpoint one. The most specific one that is for the endpoint's class applies, else the default policy. Global or same-client policies only (constraint triggers). |
| **PatchPolicy** | `Id`, `ClientId?`, `Name`, `Description?`, `AppliesTo` (`All`, `Server`, `Workstation`), `Enabled`, `ScheduleKind` (`Weekly`, `MonthlyDay`, `MonthlyWeekday`), `WeekDays`, `MonthDay` (1-31), `MonthWeek` (1-4), `MonthWeekday`, `StartMinute` (0-1439), `EndpointLocalTime`, `Scope` (`All`/`Filtered`), `UpdateSources[]`, `UpdateTypes[]`, `Severities[]`, `ExcludedNames[]`, `ExcludedVendors[]`, `RequireApproval`, `InstallDelayDays` (0-90), `AutoReboot`, `RebootMessage`, `RebootMessageText?`, `RebootTimeoutMinutes` (1-1440), `RetryHours` (1-168), `CopiedFromId?` | When and how the patch management product installs updates (0.6.0), handed to Action1 as a scheduled automation per client (§4, Patch policies). Every option is one the Action1 API offers; the allowed filter values are Action1's own (`PatchPolicyRules`). `ClientId` null = global, immutable (trigger). Check constraints on every range. |
| **ClientPatchPolicy**, **SitePatchPolicy**, **EndpointPatchPolicy** | as the policy links, with `PatchPolicyId` and the class slot `AppliesTo` | The patch policy of a client, site or endpoint (0.6.0), per class like the policy; the most specific one for the endpoint's class applies, and without any Fleeto schedules nothing for the endpoint. |
...
| **MonitoringTemplate** | `Id`, `ClientId?`, `Name`, `AppliesTo` (0.6.0: `All`, `Server`, `Workstation`) | Named set of `CheckDefinition`s with thresholds and alert rules. `ClientId` null = global. A template for one class runs none of its checks on the other, on top of the class of each check. |
...
over its client's; without any link the default policy applies, and no patch policy at all. Each policy, patch policy and
monitoring template is for all endpoints, servers or workstations (`AppliesTo`, the class override counts), and a link only
counts for an endpoint of that class: one for servers never reaches a workstation, wherever it is linked. A client or site
holds one policy and one patch policy for every endpoint, or one per class (the class slot of the link); within a level
the slot for the class wins. The default policy is for every endpoint (check constraint). Monitoring templates add up:
...
**Remote control only where there is a desktop (0.6.0, decided 2026-09-25).** The agent reports with its inventory whether the
endpoint has a graphical desktop (`Inventory.desktop`). On Linux it is `graphical` when a display manager is installed
(`/etc/systemd/system/display-manager.service` or `/etc/X11/default-display-manager`), the active session of seat0 is X11, Wayland
or Mir, or an X server runs; `none` otherwise, which is the usual Linux server. On Windows it is `none` for Server Core and Nano Server
(`InstallationType` in the registry) and `graphical` otherwise. The gateway stores only those two values. Where it is `none`, the
right-click menu of the endpoint list leaves remote control out and web refuses to open it, with remote background as the way in;
an empty value (an agent older than 0.6.0) keeps remote control as before. The signer does not check it: a desktop is a capability,
not a permission, a session on an endpoint without one only fails to show anything, and the signer then needs no access to inventory
(personal data). Remote control and remote background open from the right-click menu only; the endpoint detail has no buttons for
them (0.6.0).
...
(`CreateTenant`: map the unmapped organization named `[CODE] Name` or, failing that, with the client's name, or
`POST /organizations`, and write the mapping),
...
2 minutes to an hour. Everything else is compared with what the workers last sent: an organization name made of the client
code and name (`[ACME] Acme Corporation`, cut at 200 characters; the code is unique and never changes) that differs from
