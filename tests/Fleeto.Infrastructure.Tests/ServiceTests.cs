...
using Tier = Fleeto.Protocol.Agent.V1.Tier;
...
public class ServiceTests
{
    ...
    public async Task Grant_script_applies_cleanly_when_the_roles_exist()
    {
        ...
        await using var check = _db.DataSource.CreateCommand(
            """SELECT has_column_privilege('fleeto_web', '"InstanceSigningKeys"', 'EncryptedPrivateKey', 'SELECT'), has_column_privilege('fleeto_web', '"InstanceSigningKeys"', 'PublicKey', 'SELECT'), has_table_privilege('fleeto_gateway', '"Clients"', 'SELECT')""");
