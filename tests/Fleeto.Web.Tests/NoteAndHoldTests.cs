...
public sealed class NoteAndHoldTests
{
    ...
    public async Task Notes_are_listed_newest_first_edited_by_their_author_and_deleted_by_an_admin()
    {
        ...
        var page = await Notes.ListAsync(WebFixture.CallerWith(SystemClientScope.Instance, FleetoRoles.ReadOnly), endpoint.Id);
    ...
    public async Task Notes_are_refused_on_agent_only_endpoints_and_for_another_client()
    {
        ...
        var restricted = WebFixture.CallerWith(new RestrictedClientScope([clientA.Id]), FleetoRoles.Admin);
    ...
    public async Task A_held_alert_is_left_out_of_open_counts_and_lists_until_its_hold_ends()
    {
        ...
        Assert.False((await Alerts.HoldAsync(WebFixture.CallerWith(SystemClientScope.Instance, FleetoRoles.ReadOnly), alertId, Now.AddHours(1))).Success);
