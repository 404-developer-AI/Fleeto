...
public sealed class SignerFixture : IAsyncLifetime
{
    ...
    /// <summary>
    /// Contexts that connect as the <c>fleeto_signer</c> role with the production grants, so every test also proves
    ...
    /// </summary>
    public IFleetoDbContextFactory SignerDbFactory { get; private set; } = null!;
    ...
    public async Task InitializeAsync()
    {
        ...
        await using (var command = Database.DataSource.CreateCommand("""
            DO $$ BEGIN
              IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'fleeto_web') THEN CREATE ROLE fleeto_web NOLOGIN; END IF;
              IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'fleeto_gateway') THEN CREATE ROLE fleeto_gateway NOLOGIN; END IF;
              IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'fleeto_signer') THEN CREATE ROLE fleeto_signer NOLOGIN; END IF;
              IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'fleeto_workers') THEN CREATE ROLE fleeto_workers NOLOGIN; END IF;
            END $$;
            """))
        {
        ...
        SignerDbFactory = new FleetoDbContextFactory(FleetoDbContextFactory.BuildOptions(_signerDataSource));
