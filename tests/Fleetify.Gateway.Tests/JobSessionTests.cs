...
using JobResult = Fleetify.Protocol.Agent.V1.JobResult;
using JobStream = Fleetify.Protocol.Agent.V1.JobStream;
...
public sealed class JobSessionTests
{
    ...
    {
        var user = await _fixture.Database.CreateUserAsync(FleetifyRoles.Technician);
