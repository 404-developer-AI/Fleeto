...
public sealed class WatchdogSessionTests
{
    ...
    public async Task A_watchdog_session_runs_next_to_the_agent_and_never_handles_agent_data()
    {
        ...
        {
            CheckResults = new CheckResultBatch { Sequence = 1, Results = { new Fleeto.Protocol.Agent.V1.CheckResult { CheckId = Guid.NewGuid().ToString(), Value = 1 } } }
    ...
    public async Task A_release_is_offered_only_with_a_valid_signature_and_follows_the_ring_and_the_controls()
    {
        ...
        await File.WriteAllBytesAsync(Path.Combine(harness.BinariesDirectory, "windows-amd64", "fleeto-agent.exe"), binary);
        await File.WriteAllBytesAsync(Path.Combine(harness.BinariesDirectory, "windows-amd64", "fleeto-watchdog.exe"), [1, 2, 3]);
        var manifest = Encoding.UTF8.GetBytes($$"""
            {"formatVersion":1,"version":"{{version}}","agentBinaries":[
              {"component":"agent","platform":"windows","architecture":"amd64","file":"windows-amd64/fleeto-agent.exe","sha256":"{{Convert.ToHexStringLower(SHA256.HashData(binary))}}","size":{{binary.Length}}},
              {"component":"watchdog","platform":"windows","architecture":"amd64","file":"windows-amd64/fleeto-watchdog.exe","sha256":"{{new string('0', 64)}}","size":3}]}
            """);
        ...
        Assert.True(release.Binaries.ContainsKey("windows-amd64/fleeto-agent.exe"));
        Assert.False(release.Binaries.ContainsKey("windows-amd64/fleeto-watchdog.exe"));
        await using (var db = _fixture.Database.DbFactory.CreateSystem())
        {
        ...
        Assert.Equal(StatusCodes.Status401Unauthorized, await DownloadAsync(handler, null, version, "windows-amd64/fleeto-agent.exe"));
        using var certificate = credential.PublicCertificate();
        Assert.Equal(StatusCodes.Status404NotFound, await DownloadAsync(handler, certificate, version, "windows-amd64/fleeto-watchdog.exe"));
        Assert.Equal(StatusCodes.Status404NotFound, await DownloadAsync(handler, certificate, "0.0.1", "windows-amd64/fleeto-agent.exe"));
        Assert.Equal(StatusCodes.Status404NotFound, await DownloadAsync(handler, certificate, version, "../manifest.json"));
        Assert.Equal(StatusCodes.Status200OK, await DownloadAsync(handler, certificate, version, "windows-amd64/fleeto-agent.exe"));
    }
