...
public sealed class ScriptCheckConfigTests
{
    ...
    public async Task A_script_check_runs_the_version_its_policy_allows_and_otherwise_says_why_it_cannot_run()
    {
        ...
        var author = await _db.CreateUserAsync(FleetifyRoles.Admin);
        var approver = await _db.CreateUserAsync(FleetifyRoles.Admin);
        ...
        var adminRole = await context.Roles.Where(r => r.NormalizedName == FleetifyRoles.Admin.ToUpperInvariant()).Select(r => r.Id).SingleAsync();
