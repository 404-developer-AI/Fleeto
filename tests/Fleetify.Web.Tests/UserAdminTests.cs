...
public class UserAdminTests
{
    ...
    public async Task Last_admin_cannot_be_deleted_or_demoted()
    {
        ...
        foreach (var other in (await service.ListAsync(caller)).Where(u => u.Id != adminId && u.Roles.Contains(FleetifyRoles.Admin)))
        {
            var demoted = await service.SetRolesAsync(caller, other.Id, [FleetifyRoles.Technician]);
            Assert.True(demoted.Success, demoted.Problem);
        }

        var demote = await service.SetRolesAsync(caller, adminId, [FleetifyRoles.Technician]);
        ...
        await using (var scope = _fixture.Services.CreateAsyncScope())
        {
            ...
            Assert.True(await users.IsInRoleAsync(admin!, FleetifyRoles.Admin));
        }
        ...
        Assert.True((await service.SetRolesAsync(caller, adminId, [FleetifyRoles.ReadOnly])).Success);
    }
    ...
    public async Task Technicians_cannot_manage_users_and_passwords_need_twelve_characters()
    {
        ...
        var refused = await service.CreateAsync(technician, "tech@example.com", "Tech", "a long temporary password", [FleetifyRoles.Admin]);
        ...
        var weak = await service.CreateAsync(WebFixtureBase.Admin(), $"weak-{Guid.NewGuid():N}@example.com", "Weak", "short", [FleetifyRoles.ReadOnly]);
    ...
    public async Task Reset_two_factor_turns_it_off_and_removes_the_key()
    {
        var service = _fixture.Services.GetRequiredService<UserAdminService>();
        var caller = WebFixtureBase.Admin();
        var created = await service.CreateAsync(caller, $"reset-{Guid.NewGuid():N}@example.com", "Reset", "a long temporary password", [FleetifyRoles.Technician]);
        Assert.True(created.Success, created.Problem);
